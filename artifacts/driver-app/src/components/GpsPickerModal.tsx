      ...
      >
        ...
        <div className="px-5 pt-5 pb-3 flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-wide" style={{ color: TC }}>{t("gps_choose")}</p>
              <p className="text-xs mt-0.5 truncate max-w-[220px]" style={{ color: BROWN_MID }}>{label}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-5 pb-3">
          ...
          >
            ...
            <span className="text-sm font-medium truncate" style={{ color: BROWN }}>{address}</span>
          </div>
        </div>
        <div className="px-5 pb-6 space-y-3">
          {GPS_OPTIONS.map((opt) => (
            ...
            >
              ...
              <span className="font-bold text-base" style={{ color: BROWN }}>{opt.name}</span>
              <span className="ml-auto text-xs font-semibold" style={{ color: opt.color }}>Ouvrir →</span>
            </button>
          ))}
        </div>
      </div>
